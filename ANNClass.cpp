...
private:
	...
	void PrintVector(std::vector<double> &v) { // debug function
		printf("/n");
		for (unsigned int i = 0; i < v.size(); ++i)
			printf("%lf ", v[i]);
		printf("/n");
...
public:
	...
	ANN() { // constructor
		a = 0.01; // learning speed parameter
		eps = 0.1; // learning parameter
		NC = 2; // number of neurons in hidden layer
	}

	void Network(std::vector<double> &in, std::vector<double> &out,
		std::vector<double> &y) {
