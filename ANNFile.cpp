...
public:
	...
	int ReadInputFile(const char *file_name) 
	{
		FILE *in; // input file
	...
	int SaveWeights(std::vector<std::vector<double> > &w, wchar_t *file_name) 
	{
		...
		int SaveWeights(std::vector<std::vector<double> > &w, const char *file_name) 
	{
		FILE *out; // output file
	...
	int OpenWeights(std::vector<std::vector<double> > &w, const char *file_name)
	{
		FILE *in; // input file
		if ((in = fopen(file_name, "r")) == NULL) 
		{
	...
	int SaveOutputFile(std::vector<std::vector<double> >&v, const char *file_name)
	{
		FILE *out; // output file
		if ((out = fopen(file_name, "w+")) == NULL)
		{
